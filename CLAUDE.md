...
- **Never `git push` without explicit user permission per push.** Prior approval doesn't carry over. **Neither `git commit` in main branch without explicit user permission per push.**
